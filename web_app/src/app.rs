use crate::{TERMINAL, console_log};
use js_sys::Function;
use ratatui::{
    prelude::*,
...
impl Component for TermApp {
    ...
    fn create(ctx: &Context<Self>) -> Self {
        ...
        let cb = ctx.link().callback(|e : KeyboardEvent| TermAppMsg::KeyDown(e));
        let func: Function = Closure::<dyn 'static + Fn(KeyboardEvent)>::new(move |e : KeyboardEvent| cb.emit(e))
            .into_js_value()
            .into();
        window.set_onkeydown(Some(&func));

        let cb = ctx.link().callback(|e : KeyboardEvent| TermAppMsg::KeyUp(e));
        let func: Function = Closure::<dyn 'static + Fn(KeyboardEvent)>::new(move |e : KeyboardEvent| cb.emit(e))
            .into_js_value()
            .into();
        window.set_onkeyup(Some(&func));
        Self {}
    }
    ...
    fn update(&mut self, _ctx: &Context<Self>, msg: Self::Message) -> bool {
        match msg {
            ...
            TermAppMsg::KeyDown(event) => console_log(format!("{}, {}", event.key(), event.ctrl_key())),
            TermAppMsg::KeyUp(event) => console_log(event.key())
        }
        true
    }
